...
var DB_NAME = "players_history";

var PlayerHistoryService = {
    GetHistory: async function(query) {
    ...
    UpdateHistory: async function(query, updateQuery) {
        var obj = await new Promise(function(resolve, reject) {
            MongoClient.connect(MONGO_CONNECT, function(error, client) {
                var db = client.db("riseofthegods");
                try {
    ...
    AddHistory: async function(data) {
        var obj = await new Promise(function(resolve, reject) {
            MongoClient.connect(MONGO_CONNECT, function(error, client) {
                var db = client.db("riseofthegods");
                try {
                    db.collection(DB_NAME).insertOne(data, function(error, records) {
                        ...
                        if (!records) {
                            requestResult = {
                                type: "red",
                                message: `An error occured while adding the history of player ${data.playerID} for turn ${data.turn}`
                            };
                        } else {
    ...
    DeleteHistory: async function(query) {
        var obj = await new Promise(function(resolve, reject) {
            var orders = [];
            MongoClient.connect(MONGO_CONNECT, function(error, client) {
                var db = client.db("riseofthegods");
...
module.exports = PlayerHistoryService;