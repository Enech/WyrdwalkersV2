    ...
    <v-parallax dark src="@/assets/ParallaxAccueil.webp" eager>
      <v-row align="center" justify="center">
        <v-col class="text-center" cols="12">
          ...
          <h3 class="headline">Nos ateliers de <i>storytelling</i></h3>
        </v-col>
      </v-row>
    </v-parallax>
    <div v-if="workshops.length > 0">
      <v-row align="center" class="pa-3 pb-0">
        <v-col cols="12" md="3" v-for="(work, index) in workshops" :key="index">
          <v-card>
            <v-card-title>
            ...
            <v-card-actions>
                <v-btn text color="red" @click="showDialog = true; selectedWorkshop = work;">En savoir plus</v-btn>
                <v-spacer></v-spacer>
                <span class="mr-3"><v-icon small left>fa-calendar-alt</v-icon><span class="caption font-weight-light">{{work.date}}</span></span>
                <span><v-icon small left>fa-map-marker-alt</v-icon><span class="caption font-weight-light">{{work.location}}</span></span>
            </v-card-actions>
          </v-card>
        </v-col>
      </v-row>
      <v-dialog v-model="showDialog" fullscreen>
        ...
        <v-card-text class="mt-3">
            ...
            <v-divider></v-divider>
            <div>
                <v-icon left>fa-calendar-alt</v-icon> Le {{selectedWorkshop.date}}
            </div>
            <div>
                <v-icon left>fa-map-marker-alt</v-icon> {{selectedWorkshop.location}}
            </div>
        </v-card-text>
      </v-card>
      </v-dialog>
